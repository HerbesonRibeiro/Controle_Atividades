...
class NovoUsuarioView:
    ...
    def _criar_campos(self):
        ttk.Label(self.frame, text="Nome completo:").pack(anchor="w")
        ttk.Entry(self.frame, textvariable=self.vars["nome"]).pack(fill="x", pady=5)

        ttk.Label(self.frame, text="E-mail:").pack(anchor="w")
        ttk.Entry(self.frame, textvariable=self.vars["email"]).pack(fill="x", pady=5)

        ttk.Label(self.frame, text="Usuário:").pack(anchor="w")
        ttk.Entry(self.frame, textvariable=self.vars["usuario"]).pack(fill="x", pady=5)

        ttk.Label(self.frame, text="Senha:").pack(anchor="w")
        ttk.Entry(self.frame, textvariable=self.vars["senha"], show="*").pack(fill="x", pady=5)

        ttk.Label(self.frame, text="Cargo:").pack(anchor="w")
        ttk.Entry(self.frame, textvariable=self.vars["cargo"]).pack(fill="x", pady=5)

        ttk.Label(self.frame, text="Status:").pack(anchor="w")
        status_combo = ttk.Combobox(self.frame, textvariable=self.vars["status"], values=["ativo", "inativo"])
        status_combo.pack(fill="x", pady=5)

        ttk.Label(self.frame, text="Setor:").pack(anchor="w")
        self.combo_setor = ttk.Combobox(self.frame, textvariable=self.vars["setor"])
        self.combo_setor.pack(fill="x", pady=5)

        ttk.Button(self.frame, text="Salvar", command=self._salvar).pack(pady=20)
        ...
        try:
            with Database().get_connection() as conn:
                with conn.cursor(dictionary=True) as cursor:
                    ...
                    self.setores = {row['nome_setor']: row['id'] for row in setores}
                    self.combo_setor['values'] = list(self.setores.keys())

                    if self.setores:
                        self.vars["setor"].set(next(iter(self.setores)))  # Define o primeiro setor como padrão
